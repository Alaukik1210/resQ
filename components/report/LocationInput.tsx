...
const [AddressAutofill, setAddressAutofill] = useState<ComponentType<AddressAutofillProps> | null>(null);

  useEffect(() => {
    setMounted(true);
    // Dynamically import AddressAutofill only on client side
    import("@mapbox/search-js-react")
      .then((mod) => {
        setAddressAutofill(() => mod.AddressAutofill);
      })
      .catch((error) => {
        console.error("Failed to load AddressAutofill:", error);
      });
  }, []);
